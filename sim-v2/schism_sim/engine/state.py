...
from schism_sim.engine.barracks import BarracksJobType, BarracksOutput, BarracksState
from schism_sim.engine.combat import CombatTick, RaidCombatSession, RaidFactor, calculate_power, start_raid_session
from schism_sim.engine.logging import Event, TopFactor
from schism_sim.engine.logistics import LogisticsState
...
from schism_sim.engine.production import ProductionJobType, ProductionOutput, ProductionState
from schism_sim.engine.rules import RulesError, Ruleset
from schism_sim.engine.services.logistics import LogisticsService
from schism_sim.engine.types import (
    EnemyForce,
    FactionId,
    LocationId,
    Objectives,
...
class GameState:
    ...
    def new(seed: int = 1, data_dir: Path | None = None) -> "GameState":
        rng = Random(seed)
        if data_dir is None:
            data_dir = Path(__file__).resolve().parents[2] / "data" / "rules"
        try:
    ...
    def _resolve_current_phase(self) -> None:
        ...
        def log(name: str, value: float, delta: str, why: str) -> None:
            events.append(Event(name=name, phase=phase.value, value=value, delta=delta, why=why))
        ...
        if phase == OperationPhase.CONTACT_SHAPING:
            ...
            progress_delta, losses, supplies_spent, readiness_delta = self._resolve_phase3(op, log)
    ...
    def _resolve_phase1(self, op: ActiveOperation, log) -> tuple[float, int, Supplies]:
        ...
        approach_rules = self.rules.approach_axes.get(d.approach_axis, {"progress_mod": 0.0, "loss_mod": 0.0})
        prep_rules = self.rules.fire_support_prep.get(d.fire_support_prep, {"progress_mod": 0.0, "loss_mod": 0.0})
    ...
    def _resolve_phase2(self, op: ActiveOperation, log) -> tuple[float, int, Supplies]:
        ...
        posture_rules = self.rules.engagement_postures.get(d.engagement_posture, {"progress_mod": 0.0, "loss_mod": 0.0})
        ...
        if support_role and support_role.recon:
            recon_rating = (
                self.task_force.composition.support
                * support_role.recon.get("variance_reduction", 0.30)
                / 10.0
            )
        ...
        if recon_reduction > 0.01:
            log(
                "recon_variance_reduction",
                -base_variance * recon_reduction,
                "progress",
                f"Recon units reduce variance by {int(recon_reduction * 100)}%",
            )
        ...
        if walker_role and walker_role.transport_protection and infantry_role:
            ...
            if max_protected > 0:
                ...
                protection_reduction = protection_factor * infantry_role.transport_protection.get(
                    "protection_reduction", 0.40
                )
        ...
        base_loss_rate = 0.08 + 0.06 * max(0, fort - 1.0) + 0.04 * max(0, (op.sampled_enemy_strength or 1.0) - 1.0)
        losses_rate = base_loss_rate + loss_mod - protection_reduction
        losses = int(max(0, losses_rate * total_units * phase_days))

        return progress, losses, supplies_spent

    def _resolve_phase3(self, op: ActiveOperation, log) -> tuple[float, int, Supplies, float]:
        ...
        exploit_rules = self.rules.exploit_vs_secure.get(d.exploit_vs_secure, {"progress_mod": 0.0, "loss_mod": 0.0})
        ...
        if support_role and support_role.sustainment:
            ...
            if medic_reduction > 0.01:
                log("medic_sustainment", -medic_reduction, "losses", "Medics reduce casualties")
            if readiness_delta > 0.01:
                log("medic_readiness", readiness_delta, "readiness", "Medics improve recovery")
        ...
        if self.front_supplies.med_spares < med_cost:
            ...
            if med_class:
                ...
                log(
                    ...
                    (penalty - 1.0) * 0.05,
                    "losses",
                    "Insufficient med/spares increases casualties",
                )
    ...
    def _finalize_operation(self) -> None:
        ...
        if success:
            ...
            if end_state == "capture":
                ...
                self.contested_planet.enemy.fortification = max(
                    0.6, self.contested_planet.enemy.fortification - fort_reduction
                )
                self.contested_planet.enemy.reinforcement_rate = max(
                    0.0, self.contested_planet.enemy.reinforcement_rate - reinf_reduction
                )
            elif end_state == "raid":
