...
from clone_wars.engine.production import _allocate_parallel_share
...
class BarracksState:
    ...
    def queue_job(
        self, job_type: BarracksJobType, quantity: int, stop_at: LocationId = LocationId.NEW_SYSTEM_CORE
    ) -> None:
        """Queue a new barracks job."""
        if quantity <= 0:
            raise ValueError("Barracks quantity must be positive")
        if self.capacity <= 0:
            logger.warning("Barracks capacity is zero; queueing will stall until barracks are built.")
    ...
    def _collect_completed(self) -> list[BarracksOutput]:
        ...
        for i in range(len(self.jobs) - 1, -1, -1):
            ...
            if job.remaining <= 0:
                completed.append(
                    BarracksOutput(job_type=job.job_type, quantity=job.quantity, stop_at=job.stop_at)
                )
                self.jobs.pop(i)
        completed.reverse()
        return completed
