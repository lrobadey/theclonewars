"""Data-driven rules engine for Clone Wars War Sim."""
...
def _load_operation_rules(path: Path) -> dict[str, Any]:
    ...
    data = _load_json(path)
    result: dict[str, Any] = {}
    for key in ["approach_axes", "fire_support_prep", "engagement_postures", "risk_tolerances", "exploit_vs_secure", "end_states"]:
        if key not in data:
