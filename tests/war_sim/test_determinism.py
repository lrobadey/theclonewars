...
def test_reducer_blocks_advance_during_raid():
    state = _load_state()
    result = apply_action(state, StartRaid(target=OperationTarget.FOUNDRY))
    assert result.ok
    blocked = apply_action(state, AdvanceDay())
    assert blocked.ok is False


def test_determinism_raid():
    s1 = _load_state()
    s2 = _load_state()

    assert apply_action(s1, StartRaid(target=OperationTarget.FOUNDRY)).ok
    assert apply_action(s2, StartRaid(target=OperationTarget.FOUNDRY)).ok

    r1 = apply_action(s1, RaidResolve())
    r2 = apply_action(s2, RaidResolve())
